...
const handleFontChange = ({
  ...
  if (currentFont !== lastFontRef.current) {
    ...
    // If the font loads later, we can redraw, but ONLY if we are safely mounted.
    // Given we often use system fonts or preloaded CSS fonts, 
    // replacing the hardcoded 1-second delay avoids breaking mid-animation.
    if (typeof document !== 'undefined' && 'fonts' in document) {
      document.fonts.ready.then(() => {
        // We only redraw if we haven't lost context, but typically the initial renderCanvas catches it.
        // Doing cleanup mid-animation causes the flash/jump stutter!
      });
    }
  }
...
const createParticles = (
  ...
  // Wrap text up to 90% of the canvas width
  const maxWidth = canvas.width * 0.9;
  const match = font.match(/(\d+)px/);
  const fontSize = match ? parseInt(match[1], 10) : 48;
  ...
  lines.forEach((line) => {
    ...
    const lineLeft = alignment === "center" ? textX - metrics.width / 2 : alignment === "left" ? textX : textX - metrics.width;
    const lineRight = lineLeft + metrics.width;
  ...
  particles.forEach(particle => {
    ...
    if (shouldVaporize) {
      ...
      if (particle.speed === 0) {
        ...
        // Turn turning particles to mystical amber once vaporized
        particle.color = `rgba(245, 158, 11, ${particle.originalAlpha})`;
